...
INPUT="${PROJECT}/scenic/input/MacNair/MacNair_microglia_scenic_input.loom"

RESOURCE_DIR="${PROJECT}/scenic/resources"
RESULT_DIR="${PROJECT}/scenic/results/MacNair"
LOG_DIR="${PROJECT}/scenic/logs/MacNair"
...
ADJACENCIES="${RESULT_DIR}/MacNair_adjacencies.tsv"
REGULONS="${RESULT_DIR}/MacNair_regulons.csv"
AUC_MATRIX="${RESULT_DIR}/MacNair_regulon_auc.csv"
...
echo "Dataset: MacNair microglia"
...
"${RUNNER[@]}" \
  ...
  --method grnboost2 \
  --seed 42 \
  --output "/data/scenic/results/MacNair/MacNair_adjacencies.tsv" \
  "/data/scenic/input/MacNair/MacNair_microglia_scenic_input.loom" \
...
"${RUNNER[@]}" \
  pyscenic ctx \
  "/data/scenic/results/MacNair/MacNair_adjacencies.tsv" \
  ...
  --num_workers "${N_WORKERS}" \
  --output "/data/scenic/results/MacNair/MacNair_regulons.csv" \
...
"${RUNNER[@]}" \
  pyscenic aucell \
  "/data/scenic/input/MacNair/MacNair_microglia_scenic_input.loom" \
  "/data/scenic/results/MacNair/MacNair_regulons.csv" \
  --num_workers "${N_WORKERS}" \
  --seed 42 \
  --output "/data/scenic/results/MacNair/MacNair_regulon_auc.csv" \
  2>&1 | tee "${LOG_DIR}/03_aucell.log"

test -s "${AUC_MATRIX}"

echo
echo "MacNair pySCENIC completed."
