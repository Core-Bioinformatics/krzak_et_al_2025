...
INPUT="${PROJECT}/scenic/input/Absinta/Absinta_microglia_scenic_input.loom"

RESOURCE_DIR="${PROJECT}/scenic/resources"
RESULT_DIR="${PROJECT}/scenic/results/Absinta"
LOG_DIR="${PROJECT}/scenic/logs/Absinta"
...
ADJACENCIES="${RESULT_DIR}/Absinta_adjacencies.tsv"
REGULONS="${RESULT_DIR}/Absinta_regulons.csv"
AUC_MATRIX="${RESULT_DIR}/Absinta_regulon_auc.csv"
...
echo "Dataset: Absinta microglia"
...
"${RUNNER[@]}" \
  ...
  --method grnboost2 \
  --seed 42 \
  --output "/data/scenic/results/Absinta/Absinta_adjacencies.tsv" \
  "/data/scenic/input/Absinta/Absinta_microglia_scenic_input.loom" \
...
"${RUNNER[@]}" \
  pyscenic ctx \
  "/data/scenic/results/Absinta/Absinta_adjacencies.tsv" \
  ...
  --num_workers "${N_WORKERS}" \
  --output "/data/scenic/results/Absinta/Absinta_regulons.csv" \
...
"${RUNNER[@]}" \
  pyscenic aucell \
  "/data/scenic/input/Absinta/Absinta_microglia_scenic_input.loom" \
  "/data/scenic/results/Absinta/Absinta_regulons.csv" \
  --num_workers "${N_WORKERS}" \
  --seed 42 \
  --output "/data/scenic/results/Absinta/Absinta_regulon_auc.csv" \
  2>&1 | tee "${LOG_DIR}/03_aucell.log"

test -s "${AUC_MATRIX}"

echo
echo "Absinta pySCENIC completed."
